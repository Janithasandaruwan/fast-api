...
def test_delete_vote(authorized_client, test_post, test_vote):
    res = authorized_client.delete("/vote/", json = {"post_id": test_post[3].id, "dir":1})
    assert res.status_code == 405

def test_delete_vote_non_exist(authorized_client, test_post):
    res = authorized_client.delete("/vote/", json = {"post_id": 1213323, "dir":1})
    assert res.status_code == 405


def test_vote_post_no_exist(authorized_client, test_post):
    res = authorized_client.post("/vote/", json={"post_id": 1213323, "dir": 1})
    assert res.status_code == 404

def test_vote_unauuthorized_user(client, test_post):
